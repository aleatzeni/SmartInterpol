...
switch(fmt)
 case {'mgh','mgz'} %----------- MGH/MGZ ------------%
  ...
  err = save_mgh(permute(mri.vol,[2 1 3 4 5]), fspec, M, mr_parms,tempdir);  
  return;
 case {'bhdr'} %----------- BHDR ------------%
  bmri.te = mri.te;
  bmri.tr = mri.tr;
...
%   hdr.intent_code     = 0;
   ...
   hdr.dim = [mri.volsize(2) mri.volsize(1) mri.volsize(3)  mri.nframes size(mri.vol,5)];
   try, hdr.intent_p1       = mri.niftihdr.intent_p1; catch ME, hdr.intent_p1 = 0; end
   try, hdr.intent_p2       = mri.niftihdr.intent_p2; catch ME, hdr.intent_p2 = 0; end
   try, hdr.intent_p3       = mri.niftihdr.intent_p3; catch ME, hdr.intent_p3 = 0; end
   try, hdr.intent_code     = mri.niftihdr.intent_code; catch ME, hdr.intent_code = 0; end
  ...
  % Note that the order is 2 1 3 4 5 6 7
  hdr.vol = permute(mri.vol,[2 1 3 4 5 6 7]);
...
hdr.dim(6) = size(hdr.vol,5);
hdr.dim(7) = size(hdr.vol,6);
hdr.dim(8) = size(hdr.vol,7);
