classdef myImageDataAugmenter < handle
    ...
    %       RandContrast        - Random contrast variation
    %       RandBrightness      - Random brightness variation
    %   Example 1
    %   ---------
    ...
    properties (SetAccess = 'private')
        ...
        %   
        %    Contrast is a two element numeric vector that specifies the
        %    random range of contrast that is applied to
        %    input image data.
        %   
        %   Default: [1 1]
        RandContrast
        
        %Brightness - Random brightness variation
        %
        %    Contrast is a two element numeric vector that specifies the
        %    random range of contrast that is applied to
        %    input image data.
    ...
    methods
        ...
        function self = myImageDataAugmenter(varargin)
            ...
            %                       Default: [0 0]
            %             
            %   'RandContrast'      A two element vector that defines the
            %                       uniform range of contrast that will
            %                       applied to the input image.
            %
            %                       Default: [1 1]
            %
            %   'RandBrightness'    A two element vector that defines the
            %                       uniform range of brightness that will
            %                       applied to the input image.
            %
            %                       Default: [-10 10]
...
%         function set.RandContrast(self,contrast)
%             
%             validateattributes(contrast,{'numeric'},{'real','finite','vector','numel',2},mfilename,'RandContrast');
%             validateRange(contrast,'RandContrast');
%             self.RandContrast = contrast;
%         end
%         
%         function set.RandBrightness(self,brightness)
%             
%             validateattributes(brightness,{'numeric'},{'real','finite','vector','numel',2},mfilename,'RandBrightness');
%             validateRange(brightness,'RandBrightness');
%             self.RandBrightness = brightness;
%         end
    ...
    methods (Access = 'private')
        ...
        function imgOut = modifyContrBright(self,imgIn)
            self.Contrast = selectUniformRandValueFromRange(self.RandContrast);
            self.Brightness = selectUniformRandValueFromRange(self.RandBrightness);
            
            imgOut=uint8(((double(imgIn)-128).*self.Contrast)+128+self.Brightness);
        end
        ...
        function parseInputs(self,varargin)
            ...
            p.addParameter('RandContrast',[1 1]);
            p.addParameter('RandBrightness',[-10 10])
            ...
            self.RandContrast = params.RandContrast;
            self.RandBrightness = params.RandBrightness;
    ...
    methods (Hidden)
        ...
        function B = augment(self,A)
            ...
            if iscell(A)    
                ...
                for img = 1:numel(A)
                    ...
                    Bout{img} = self.modifyContrBright(A{img});
                    B{img} = nnet.internal.cnnhost.warpImage2D(Bout{img},tform(1:3,1:2),'linear',fillValue);
            ...
            elseif isnumeric(A) && (ndims(A) < 4)
                ...
                Bout = self.modifyContrBright(A);
                B = nnet.internal.cnnhost.warpImage2D(Bout,tform(1:3,1:2),'linear',manageFillValue(A,self.FillValue));
      ...
      function [A, B] = augmentPair(self, X, Y, interpY, fillValueY)
       ...
       function B = augmentY(~, Y, tform, interp, fillValue)
           ...
           %Initialised as empty every time we call the augmenter
           DEFORMED_GLOBAL_POSTERIORS=[];
            % Augment using known transform.
            if iscell(Y)
                %Size of DEFORMED_GLOBAL_POSTERIORS is H x W x NClass x NObser
                DEFORMED_GLOBAL_POSTERIORS=zeros([size(GLOBAL_POSTERIORS) length(Y)]);
                B = cell(size(Y));
                for img = 1:numel(Y)
                    fillValue = manageFillValue(Y{img},fillValue);
                    
                    % Use soft posteriors with sampling
                    if ~isempty(GLOBAL_POSTERIORS) && USE_GLOBAL_POSTERIORS>0 && SAMPLING>0 && max(max(Y{img}))==0
                        LABELS=sampleFromProbs(GLOBAL_POSTERIORS);
                        LABELS=LABELS-1;
                        B{img} = nnet.internal.cnnhost.warpImage2D(LABELS,tform(1:3,1:2,img),interp,fillValue);
                    % Use soft posteriors without sampling    
                    elseif ~isempty(GLOBAL_POSTERIORS) && USE_GLOBAL_POSTERIORS>0 && SAMPLING==0 
                       
                         B{img} = nnet.internal.cnnhost.warpImage2D(Y{img},tform(1:3,1:2,img),interp,fillValue);
                        
                        if max(max(Y{img}))==0
                            DeformedGlobalPosterior=zeros(size(GLOBAL_POSTERIORS));
                            interpPosteriors='linear';
                            fillValuePosteriors=NaN;
                            for label=1:size(GLOBAL_POSTERIORS,3)
                                DeformedGlobalPosterior(:,:,label) = nnet.internal.cnnhost.warpImage2D(GLOBAL_POSTERIORS(:,:,label),tform(1:3,1:2,img),interpPosteriors,fillValuePosteriors);
                                if label==1
                                    DeformedGlobalPosterior(isnan(DeformedGlobalPosterior))=1;
                                    %set nans to ones
                                else
                                    DeformedGlobalPosterior(isnan(DeformedGlobalPosterior))=0;
                                    %set nans to zeros
                                end
                            end
                            DEFORMED_GLOBAL_POSTERIORS(:,:,:,img)=DeformedGlobalPosterior;
                            
                        else
                            
                            nclass=size(GLOBAL_POSTERIORS,3);
                            POST=zeros([size(Y{img}) nclass]);
                            [R,C]=ndgrid(1:size(Y{img},1),1:size(Y{img},2));
                            idx=sub2ind(size(POST),R(:),C(:),B{img}(:)+1);
                            POST(idx(~isnan(idx)))=1;
                            M=isnan(B{img});
                            for c=1:nclass
                               aux=POST(:,:,c);
                               if c==1, aux(M)=1; else, aux(M)=0; end
                               POST(:,:,c)=aux;
                            end
                            DEFORMED_GLOBAL_POSTERIORS(:,:,:,img)=POST;
            ...
            elseif isnumeric(Y) && (ndims(Y) < 4)    
                ...
                if ~isempty(GLOBAL_POSTERIORS) && USE_GLOBAL_POSTERIORS>0 && SAMPLING>0 && max(max(Y))==0
                    LABELS=sampleFromProbs(GLOBAL_POSTERIORS);
                    LABELS=LABELS-1;
                    B = nnet.internal.cnnhost.warpImage2D(LABELS,tform(1:3,1:2),interp,fillValue);
                elseif ~isempty(GLOBAL_POSTERIORS) && USE_GLOBAL_POSTERIORS>0 && SAMPLING==0 
                    ...
                    if max(max(Y))==0
                            ...
                            fillValuePosteriors=NaN;
                            for label=1:size(GLOBAL_POSTERIORS,3)
                                DeformedGlobalPosterior(:,:,label) = nnet.internal.cnnhost.warpImage2D(GLOBAL_POSTERIORS(:,:,label),tform(1:3,1:2),interpPosteriors,fillValuePosteriors);
                                if label==1
                                    DeformedGlobalPosterior(isnan(DeformedGlobalPosterior))=1;
                                    %set nans to ones
                                else
                            ...
                            DEFORMED_GLOBAL_POSTERIORS=DeformedGlobalPosterior;
                            
                        else
                            nclass=size(GLOBAL_POSTERIORS,3);
                            POST=zeros([size(Y) nclass]);
                            [R,C]=ndgrid(1:size(Y,1),1:size(Y,2));
                            idx=sub2ind(size(POST),R(:),C(:),B(:)+1);
                            POST(idx(~isnan(idx)))=1;
                            M=isnan(B);
                            for c=1:nclass
                               aux=POST(:,:,c);
                            ...
                            DEFORMED_GLOBAL_POSTERIORS=POST;
                    end
                else
                    B = nnet.internal.cnnhost.warpImage2D(Y,tform(1:3,1:2),interp, manageFillValue(Y,fillValue));
                end
    ...
    methods(Static, Hidden = true)
        function self = loadobj(S)
            self = imageDataAugmenter('FillValue',S.FillValue,...
                ...
                'RandContrast',S.RandContrast,...
                'RandBrightness',S.RandBrightness);
    ...
    methods (Hidden)
        function S = saveobj(self)
            ...
            S = struct('FillValue',self.FillValue,...
                ...
                'RandContrast',self.RandContrast,...
                'RandBrightness',self.RandBrightness);
        end
